...
#    fresh_when(@movie)
    @movie.movie_accesses.create(:action=>"cached-action#show")
    if stale? @movie
      @movie.movie_accesses.create(:action=>"cached-action#show-stale")
...
#     response.headers["Cache-Control"] = "public, max-age=#{secs}"
  ...
  def create
    ...
    respond_to do |format|
      if @movie.save
        fresh_when(@movie)
        @movie.movie_accesses.create(:action=>"cached-action#create")
  ...
  def update
    respond_to do |format|
      if @movie.update(movie_params)
        ...
        fresh_when(@movie)
        @movie.movie_accesses.create(:action=>"cached-action#update")
